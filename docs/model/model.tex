...
\newcommand{\stat}[3]{%
  ...
  {\fontsize{26}{28}\selectfont\bfseries\color{accent}#2}\\[0.3em]
  {\small\color{muted}#3}
  \end{minipage}}
...
\vspace{-0.2em}
\begin{center}
\includegraphics[width=0.86\textwidth,trim=0 0 0 0,clip]{../../figures/spikeGrid.png}
\end{center}
\vspace{-0.4em}
\begin{center}
\stat{0.26\textwidth}{9 / 9}{release dates show a jump}\hfill
\stat{0.26\textwidth}{5.6$\times$}{median vs.\ a normal day}\hfill
\stat{0.26\textwidth}{both ways}{sign follows the surprise}
\end{center}
\end{frame}
...
\node[draw,rounded corners,fill=wash,inner sep=9pt,text width=3.4cm,align=center] (a)
  {big surprise\\[0.2em]\lead{+40 bp}};
\node[draw,rounded corners,fill=wash,inner sep=9pt,text width=3.4cm,align=center,right=1.6cm of a] (b)
  {dull release\\[0.2em]\lead{+2 bp}};
\node[draw=warn,line width=1.2pt,rounded corners,fill=warn!8,inner sep=9pt,text width=4.0cm,align=center,right=1.6cm of b] (c)
...
\vspace{1.6em}
\begin{center}
{\color{muted}It is the fastest route to a clean-looking wrong answer,\\
...
\begin{center}
\begin{tikzpicture}[font=\small]
\node[draw,rounded corners,fill=wash,inner sep=11pt,text width=4.6cm,align=center] (A)
  {\lead{$A_e$}\ \ amplitude\\[0.3em]{\color{muted}how big was the news}\\[0.2em]{\color{muted}\itshape don't care}};
\node[draw=accent,line width=1.2pt,rounded corners,fill=accent!8,inner sep=11pt,text width=4.6cm,align=center,right=1.4cm of A] (L)
  {\lead{$\lambda_e$}\ \ rate\\[0.3em]{\color{muted}how fast it was absorbed}\\[0.2em]{\color{accent}\itshape the answer}};
\end{tikzpicture}
\end{center}
\vspace{1.0em}
$$\mathbb{E}\big[R_e(\tau)\big]=A_e\big(1-e^{-\lambda_e\tau}\big)
\qquad\Longrightarrow\qquad
\phi_e(\tau)=\frac{\ \sout{A_e}\,\big(1-e^{-\lambda_e\tau}\big)}{\sout{A_e}}$$
\vspace{0.4em}
\begin{center}
\ok{The amplitude cancels on paper.} A dull event now gives a \emph{wide} answer,\\
{\color{muted}which is the honest response to weak evidence. Nothing is dropped.}
\end{center}
\end{frame}
...
\begin{center}
\begin{tikzpicture}[font=\small]
\node[draw,rounded corners,fill=wash,inner sep=10pt,text width=3.5cm,align=center] (n)
  {\textbf{No pooling}\\[0.4em]fit each event alone\\[0.5em]\bad{214 shrugs}\\{\color{muted}\scriptsize every interval useless}};
\node[draw,rounded corners,fill=wash,inner sep=10pt,text width=3.5cm,align=center,right=0.7cm of n] (c)
  {\textbf{Complete pooling}\\[0.4em]one $\lambda$ for all\\[0.5em]\bad{one number}\\{\color{muted}\scriptsize CPI vs FOMC? 2017 vs 2026?}};
\node[draw=good,line width=1.4pt,rounded corners,fill=good!8,inner sep=10pt,text width=3.5cm,align=center,right=0.7cm of c] (p)
...
\vspace{0.4em}
\begin{center}
{\color{muted}Every event keeps its own rate --- and all the rates come from one family.}
\end{center}
\end{frame}
...
\begin{center}
\begin{tikzpicture}
\node[draw=accent,line width=1.1pt,rounded corners,fill=accent!6,inner sep=9pt,text width=0.82\textwidth,align=center]
  {\lead{$\sigma$ is estimated too.}\ \ Events really differ $\Rightarrow$ $\sigma$ grows, pooling weakens.\\
...
\node[draw,rounded corners,fill=wash,inner sep=9pt,text width=4.7cm,align=left] (a)
  {$A_e$ given $\lambda_e$\hfill\ok{closed form}\\[0.2em]{\color{muted}\scriptsize plain linear regression}};
\node[draw=warn,line width=1.1pt,rounded corners,fill=warn!8,inner sep=9pt,text width=4.7cm,align=left,below=0.35cm of a] (b)
  {$\lambda_e$ given $A_e$\hfill\bad{none}\\[0.2em]{\color{muted}\scriptsize $\lambda$ sits inside $e^{-\lambda\tau}$}};
\node[draw=warn,line width=1.1pt,rounded corners,fill=warn!8,inner sep=9pt,text width=4.7cm,align=left,below=0.35cm of b] (c)
  {the joint posterior\hfill\bad{none}\\[0.2em]{\color{muted}\scriptsize 400+ parameters, no name}};
...
% ---------------------------------------------------------------- 12
\begin{frame}{Yes, it rhymes with TrueSkill. Up to a point.}
\vspace{0.7em}
\begin{columns}[T,onlytextwidth]
\begin{column}{0.47\textwidth}
{\color{good}\bfseries Same shape}
\vspace{0.5em}
\begin{itemize}\setlength\itemsep{0.45em}\small
\item latent quantity per entity
\item population prior over it
\item evidence updates it
\item shrinkage carries the thin cases
\end{itemize}
\end{column}
\begin{column}{0.47\textwidth}
{\color{warn}\bfseries Different engine}
\vspace{0.5em}
\begin{itemize}\setlength\itemsep{0.45em}\small
\item TrueSkill: every message stays Gaussian, so \lead{EP works}
\item ours: the likelihood is nonlinear in $\lambda$
\item the messages have no closed form
\item \lead{so: sampling, not message passing}
\end{itemize}
\end{column}
\end{columns}
\vspace{1.2em}
\begin{center}
{\color{muted}It is a factor graph. It is just not fitted by passing messages around it.}
\end{center}
\end{frame}
...
{\small\textbf{one number}}\\{\scriptsize\color{muted}half-life in seconds,\\with an interval}
\end{column}
\begin{column}{0.31\textwidth}
\centering
\vspace{0.7em}
