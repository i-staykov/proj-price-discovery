...
{\scriptsize\color{muted}The denominator makes $\phi(H)$ exactly 1. For fast rates it is 1 to
machine precision.}
\vspace{0.7em}
...
\node[draw,rounded corners,fill=wash,inner sep=11pt,text width=4.6cm,align=center] (A)
  {\lead{$M_e$}\ \ size of the move\\[0.3em]{\color{muted}how big was the news}\\[0.2em]{\color{muted}\itshape don't care}};
...
\phi_e(\tau)=\frac{\ \sout{M_e}\,\dfrac{1-e^{-\lambda_e\tau}}{1-e^{-\lambda_e H}}}{\sout{M_e}}$$
\vspace{0.3em}
\begin{center}
\ok{The size cancels on paper.} A dull event now gives a \emph{wide} answer,\\
